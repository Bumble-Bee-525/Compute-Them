...
def run(currentState):
    ...
    while i < len(currentState["code"]):
        ...
        #if it's a function, keep track
        if line[0:8] == "function":
            layers += 1
            funName = line[8:line.find("(")]
            currentBlock = ""
        
        if layers == 0:
            if funName != "":
                functions[funName] = currentBlock[:-2]
                funName = ""
            else:
    ...
    for block in segmentedCode:
        ...
        if keyword == "for":
            ...
            times = int(block[block.find("<") + 1:block.find(";", block.find("<"))])
        ...
        elif keyword in functions.keys():
            #find the function declaration
            currentState["code"] = functions[keyword]
            for funName in functions:
                currentState["code"] += "\nfunction" + funName + "(){\n" + functions[funName] + "}\n"
            currentState = run(currentState)
...
#get mode
autoMode = pg.confirm(text='Activate auto mode?', title="Mode Selection", buttons=["Yes", "No balls"]) == "Yes"
start = time()

#main loop
keepGoing = True
while keepGoing and levelnum < 60:
    ...
    #special cases for super duper fights
    if levelnum > 56:
        names = ["Oddities/pre_boss.txt", "Oddities/pre_boss_2.txt", "Oddities/boss.txt"]
        level = ""
        bossFile = open(names[levelnum - 57])
    ...
    if not autoMode:
        keepGoing = pg.confirm(text=f"Do you wanna keep going? \nMost recent level complete: {levelnum}", title="no balls?", buttons=["Yes", "No"]) == "Yes"
    else:
        #2.6 seconds minimum   
        sleep(2.6)
    levelnum += 1
...
#if player wins
if levelnum == 60:
    pg.alert(text="Congratulations! You beat the game!", title="THE END", button="Dang bro")

end = time()
timeElapsed =  end - start
min = floor(timeElapsed / 60)
seconds = (timeElapsed % 60)
seconds = round(seconds)

pg.alert(text=f"Your score: {levelnum}\nTime taken: {min}:{seconds}", title="THE END", button="Alright")
